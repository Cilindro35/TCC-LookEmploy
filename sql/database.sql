...
-- 11) TABELA AvaliacaoPrestador
CREATE TABLE IF NOT EXISTS AvaliacaoPrestador (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    INDEX idx_ap_prestador (prestador_id),
    INDEX idx_ap_cliente (cliente_id),
    INDEX idx_ap_servico (servico_id),
    CONSTRAINT fk_ap_prestador FOREIGN KEY (prestador_id) REFERENCES Prestador(ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_ap_cliente FOREIGN KEY (cliente_id) REFERENCES Cliente(ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_ap_servico FOREIGN KEY (servico_id) REFERENCES Servico(codigoServico) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 12) VIEW para estatísticas de segurança
...
-- 15) Trigger para normalizar ordem de pares em conversas (garante user_a < user_b)
